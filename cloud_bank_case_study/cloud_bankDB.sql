...
CREATE TABLE cloud_bank.regions (
  region_id INT PRIMARY KEY,
  region_name VARCHAR(50)
);

INSERT INTO cloud_bank.regions (region_id, region_name)
VALUES
  (1, 'Australia'),
  (2, 'America'),
  (3, 'Africa'),
...
CREATE TABLE cloud_bank.customer_nodes (
  customer_id INT,
  region_id INT,
  node_id INT,
  start_date DATE,
...
INSERT INTO cloud_bank.customer_nodes (customer_id, region_id, node_id, start_date, end_date)
VALUES
...
CREATE TABLE cloud_bank.customer_transactions (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
INSERT INTO cloud_bank.customer_transactions
...
The financial industry is undergoing a digital revolution with the rise of Neo-Banks — modern, 
fully digital banks that operate without physical branches.

Amidst this transformation, We envisioned a futuristic blend of digital banking, 
cloud technology, and the world of data. 
This vision gave rise to Cloud Bank — a next-generation platform that combines traditional banking with distributed,
secure cloud storage.

Cloud Bank works like any other digital bank for day-to-day financial activities. 
However, it also offers a powerful twist: each customer is provided with cloud data storage—and the 
amount of storage is dynamically linked to their account balance.
 
This hybrid banking-cloud model opens up new frontiers, 
but also presents unique challenges. That’s where you come in.
...
select count(distinct node_id) count_unique_nodes from cloud_bank.customer_nodes;

--What is the distribution of nodes across different regions?
with count_details as(
select c.region_id,c.node_id, count(c.node_id) as counts, (select count(m.node_id) from cloud_bank.customer_nodes as m where m.region_id = c.region_id group by region_id) as total_count from cloud_bank.customer_nodes as c
group by region_id, node_id)
select c.region_id,r.region_name, c.node_id,c.counts, round(cast(counts as float)/cast(total_count as float)*100.0, 2) as dist_of_nodes from count_details as c
inner join cloud_bank.regions as r
on c.region_id = r.region_id
order by c.region_id,c.node_id;

--How many customers are allocated to each region?
select c.region_id, r.region_name, count(distinct customer_id) as total_customer from cloud_bank.customer_nodes as c
inner join cloud_bank.regions as r
on c.region_id = r.region_id
...
with newCTE as (
select customer_id, DATEDIFF(day, start_date,lead(start_date) over(partition by customer_id order by start_date)) as diff from cloud_bank.customer_nodes
where end_date != '9999-12-31')
select avg(diff) as avg_days from newCTE;


--What are the median, 80th percentile, and 95th percentile reallocation durations (in days) for customers in each region?
select region_id, DATEDIFF(day, start_date,end_date) as diff, FLOOR(PERCENT_RANK() over(partition by region_id order by DATEDIFF(day, start_date,end_date))*100.0) as percentail from cloud_bank.customer_nodes
where end_date != '9999-12-31'
...
select txn_type, count(distinct customer_id) as counts, 
sum(txn_amount) as total_amount from cloud_bank.customer_transactions
group by txn_type;

--What is the average number of historical deposits per customer, along with the average total deposit amount?
select customer_id,cast(count(txn_type) as float)/cast((select count(*) from cloud_bank.customer_transactions where customer_id = customer_id) as float) as Avg_deposites, avg(txn_amount) as Avg_amount from cloud_bank.customer_transactions
...
select customer_id, count(txn_type) as counts,month(txn_date) as months from cloud_bank.customer_transactions
where txn_type = 'deposit'
...
select c1.months, count(c1.customer_id) as customer_count from customer_count as c1
inner join customer_sencond as c2
on c1.customer_id = c2.customer_id and c1.months = c2.months
group by c1.months;

-----------------------------------------------------

--What is the closing balance for each customer at the end of every month?
with recusrsive_cte as(
select distinct customer_id, 1 as months from cloud_bank.customer_transactions

union all 

select customer_id, months + 1 from recusrsive_cte
where months < 4
),

all_months as(
select customer_id, months from recusrsive_cte
),

all_amounts as (
select am.customer_id, am.months, isnull((select sum(txn_amount) from cloud_bank.customer_transactions as ct
where ct.txn_type = 'deposit' and am.customer_id = ct.customer_id and am.months = month(ct.txn_date)), 0) as deposite_amount, isnull((select sum(txn_amount) from cloud_bank.customer_transactions as ct
where ct.txn_type = 'withdrawal' and am.customer_id = ct.customer_id and am.months = month(ct.txn_date)), 0) as withdra_amount, isnull((select sum(txn_amount) from cloud_bank.customer_transactions as ct
where ct.txn_type = 'purchase' and am.customer_id = ct.customer_id and am.months = month(ct.txn_date)), 0) as purchase_amount from all_months as am
),
 
closeing_balanceCTE as (
select customer_id, months,deposite_amount, withdra_amount, purchase_amount, ((deposite_amount-withdra_amount)-purchase_amount) as closeing_balance from all_amounts)

select customer_id, months,
sum(closeing_balance) over(partition by customer_id order by months) as closeing_balance from closeing_balanceCTE 
order by customer_id;
--order by customer_id;

--select customer_id, month(txn_date) as dates, sum(txn_amount) as amounts from cloud_bank.customer_transactions
--where customer_id = 10 and txn_type = 'deposit'
--group by customer_id,month(txn_date);

--select customer_id, month(txn_date) as dates, sum(txn_amount) as amounts from cloud_bank.customer_transactions
--where customer_id = 10 and txn_type = 'withdrawal'
--group by customer_id,month(txn_date);

--select customer_id, month(txn_date) as dates, sum(txn_amount) as amounts from cloud_bank.customer_transactions
--where customer_id = 10 and txn_type = 'purchase'
--group by customer_id,month(txn_date);

--- isko mujhe windows function se try karna he running total se
...
select customer_id, months, 
case
	when closeing_balance = 0 then 1
	else closeing_balance
end as closeing_balance from all_row_closeing
where closeing_balance > 0),

percents as(
select customer_id, months, closeing_balance, round((((cast(closeing_balance as float) - lag(cast(closeing_balance as float)) over(partition by customer_id order by months))/lag(cast(closeing_balance as float)) over(partition by customer_id order by months))*100.0),2) as groth_percent from closeing_1),

--select * from percents
--order by customer_id;
groth_count as (
select c.customer_id, (select count(groth_percent) from percents as p where groth_percent > 5.0 and c.customer_id = p.customer_id) as counts from percents as c
group by customer_id)

--select * from groth_count
--where counts = 3;

select round((cast(count(distinct customer_id) as float)/cast((select count(distinct customer_id) from cloud_bank.customer_transactions ) as float))*100.0,2) as percent_of_5_groth from groth_count  
where counts = 3;
...
all_row_balance as (select customer_id, months,
sum(closeing_balance) over(partition by customer_id order by months) as closeing_balance from closeing_balanceCTE)

select customer_id, months, closeing_balance, concat(lag(closeing_balance,1,0) over (partition by customer_id order by months),' GB') as Allocated_storage from all_row_balance
order by customer_id;
...
all_row_balance as (select customer_id, months,
sum(closeing_balance) over(partition by customer_id order by months) as closeing_balance1 from closeing_balanceCTE)

select a.customer_id, a.months, a.closeing_balance1,
(select min(ct.txn_amount) from cloud_bank.customer_transactions as ct where a.customer_id = ct.customer_id and a.months = month(ct.txn_date)) as minimum_txn_amount ,
(select max(ct.txn_amount) from cloud_bank.customer_transactions as ct where a.customer_id = ct.customer_id and a.months = month(ct.txn_date)) as maximum_txn_amount, 
concat(round(cast(a.closeing_balance1 as float) / 30.0,2),' GB') as avg_Allocated_storage from all_row_balance as a
order by a.customer_id;

--Option 3: Storage is updated in real-time, reflecting the balance after every transaction
with real_balance as (
select row_number() over(PARTITION by customer_id order by customer_id) as rn, customer_id,txn_date,txn_type, txn_amount,
case
	when month(txn_date) = 1 and txn_type ='deposit' then txn_amount
	else 0 
end as real_time
from cloud_bank.customer_transactions),

sign_balance as (
select rn,customer_id, txn_date, txn_type, txn_amount,
case
	when txn_type = 'deposit' then + txn_amount
	when txn_type = 'withdrawal' then - txn_amount
	when txn_type = 'purchase' then - txn_amount
end as real_time_balance
from real_balance
)

select rn,customer_id, txn_date, txn_type, txn_amount,concat(sum(real_time_balance) over(partition by customer_id order by txn_date),' GB') as Running_allocated_storage from sign_balance
order by customer_id;
-----------------------------------------------------------
